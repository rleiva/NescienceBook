...
\begin{definition}
Let $A \subset \mathcal{T}$ be an area with known subset $\hat{A} = \{r_1, r_2, \ldots, r_n\}$, $s \in \mathcal{B}^\ast$ a string, and $d_{\hat{A} \mid s}$ a condtional description. We define the \emph{inaccuracy of the area} $d_{\hat{A}}$ as:
\[
\iota(d_{\hat{A} \mid s}) = \frac{ \max\{ K \left( \langle r_1, r_2, \ldots, r_n \rangle \mid \delta(\langle d, t \rangle) \right), K \left( \delta(\langle d, t \rangle) \mid \langle r_1, r_2, \ldots, r_n \rangle \right) \} } { \max\{ K(\langle r_1, r_2, \ldots, r_n \rangle), K \left(\delta(\langle d, t \rangle) \right) \} }
\]
\end{definition}
