...
In our definition of surfeit, we chose a relative measure rather than an absolute one (i.e., $| l(d) - K(r) |$) because we aim to compare the surfeit not only among different models of the same entity but also across models of different entities. We prefer to use $K(r)$ instead of the equivalent $l \left( d^\star \right)$, where $d^\star$ is a perfect description of $r$, to maintain consistency with the definition of inaccuracy provided in Section \ref{sec:inaccuracy:inaccuracy}.
...
Let $r \in \mathcal{B}^\ast$ be a representation, and $d \in \mathcal{D}_r$ one of its valid descriptions, then we have that $0 \leq \sigma(d, r) \leq 1$.
\end{proposition}
\begin{proof}
...
Let $r \in \mathcal{B}^\ast$ be a representation, and $d \in \mathcal{D}_r$ a valid description for $r$, then we have that  $\sigma(d, r) = 0$ if and only if $l(d) = l(d^\star_r)$.
\end{proposition}
\begin{proof}
Assume $l(d) = l(d^\star_r) = K(r)$. Then:
\[
...
\[
|l(d) - K(r)| = 0 \Rightarrow l(d) = K(r) = l(d^\star_r).
\]
\end{proof}
...
Given a description $d \in \mathcal{D}$, we define the \emph{redundancy}\index{Redundancy} of the description $d$, denoted by $\rho(d)$, as
\[
...
Let $r \in \mathcal{B}^\ast$ be a representation , and $d \in \mathcal{D}_r$ one of its valid descriptions, then we have that $\rho(d) \leq \sigma(d, r)$.
\end{proposition}
\begin{proof}
...
It would be very nice if Proposition \ref{prop:surfeit_comparison} applies to all possible descriptions. Unfortunately, the proposition is true only when we deal with valid descriptions (from $\mathcal{D}^\star_r$).
...
\begin{definition}
Let $r \in \mathcal{B}^\ast$ be a representation, $s \in \mathcal{B}^\ast$ a string, and $d \in \mathcal{D}_{r \mid s}$ a description of $r$ given $s$. We define the \emph{conditional surfeit}\index{Conditional surfeit} of the conditional description $d_{r \mid s}$, denoted by $\sigma(d_{r \mid s})$, as: 
\[
...
\end{proposition}
\begin{proof}
Given that $l \left( d_{r \mid s} \right) > 0$ and that $K\left( r \mid s \right) \geq 0$, since they are the lengths of non-empty strings, and that $l\left( d_{r \mid s} \right) \geq K\left( r \mid s \right)$.
\end{proof}

Intuition tells us that the surfeit of a description could only decrease if we assume the background knowledge given by the description of another topic. This is because we require that this background knowledge must be a perfect description (it presents no surfeit). However, as it was the case of conditional descriptions, we have to wait until Chapter \ref{chap:Nescience} to formalize this intuition.
...
\begin{definition}
Let $s \in \mathcal{B}^\ast$ be a string, and $d \in \mathcal{D}$ be a conditional description given $s$. We define the \emph{conditional redundancy}\index{Conditional Redundancy} of the conditional description $d_{t \mid s}$, denoted by $\sigma(d_{t \mid s})$, as: 
\[
\rho(d_{r \mid s}) = 1 - \frac{K \left( d_{t \mid s} \right)}{l \left( d_{t \mid s} \right)}
\]
\end{definition}

Conditional redundancy is a relative measure, and so, a number between $0$ and $1$.

\begin{proposition}
We have that $0 \leq \rho(d_{t \mid s}) \leq 1$ for all $t,s$ and all $d_{t,s}$.
\end{proposition}
\begin{proof}
Given that $K(d_{t \mid s}) \leq l(d_{t \mid s})$ we have that $\frac{K(d_{t \mid s})}{l \left( d_{t \mid s} \right)} \leq 1$ and so, $1 - \frac{K(d_{t \mid s})}{l \left( d_{t \mid s} \right)} \geq 0$. Also, since $\frac{K(d_{t \mid s})}{l \left( d_{t \mid s} \right)} > 0$ (both quantities are positive integers), we have that $1 - \frac{K(d_{t \mid s})}{l \left( d_{t \mid s} \right)} \leq 1$.
\end{proof}

Finally, we can extend our concepts of conditional surfeit and conditional redundancy to multiple, but finete, number of topics.

\begin{definition}
Let $t, s_1, s_2, \ldots, s_n \in \mathcal{D}$ be a finite collection of descriptions, and let $d_{t \mid s_1, s_2, \ldots,s_n}$ any conditional description of $t$ given $s_1, s_2, \ldots, s_n$. We define the \emph{conditional surfeit} of the description $d_{t \mid s_1, s_2, \ldots,s_n}$, denoted by $\sigma(d_{t \mid s_1, s_2, \ldots, s_n})$, as: 
\[
\sigma(d_{t \mid s_1, s_2, \ldots,s_n}) = 1 - \frac{K\left( t \mid s_1, s_2, \ldots, s_n \right)}{l \left( d_{t \mid s_1, s_2, \ldots,s_n} \right)}
\]
...
\begin{definition}
Let $d_1, d_2 \in \mathcal{D}$ be two descriptions, and $r \in \mathcal{B}^\ast$ a representation. We define the \emph{rate of change between the surfeit and inaccuracy} of the descriptions $d_1, d_2$ given the representation $r$, denoted by $\Delta_{\sigma \iota} (d_1, d_2, r)$ as:
\[
\Delta_{\sigma \iota} (d_1, d_2, r) = \frac{\sigma(d_2, r) - \sigma(d_1, r)}{\iota(d_2, r) - \iota(d_1, r)}
\] 
...
\begin{example}
Let $d_1 \in \mathcal{D}$ be a description and $r \in \mathcal{B}^\ast$ a representation with a surfeit $\sigma(d_1, r)$ of 0.35 and an inaccuracy $\iota(d_1, r)$ of 0.20. Let $d_2 \in \mathcal{D}$ be a second description with a surfeit $\sigma(d_2, r)$ of 0.30 (a slight decrease from 0.35) and an inaccuracy $\iota(d_2, r)$ of 0.2001 (a very small increase from 0.20). Applying the definition of the rate of change:
\[
...
\begin{definition}
Let $d_1 \in \mathcal{D}$ be a description and $r \in \mathcal{B}^\ast$ a representation. The description $d_1$ is said to be a Pareto point\index{Pareto point} with respect to surfeit $\sigma$ and inaccuracy $\iota$ if there does not exist another description $d_2$ such that:
\begin{enumerate}
    \item $\sigma(d_2, r) \leq \sigma(d_1, r)$ and $\iota(d_2, r) \leq \iota(d_1, r)$, and
    \item $\sigma(d_2, r) < \sigma(d_1, r)$ or $\iota(d_2, r) < \iota(d_1, r)$.
\end{enumerate}
\end{definition}

In simpler terms, a description is Pareto optimal if: (i) no other description is better in both surfeit and inaccuracy, and (ii) no other feasible description improves one metric without worsening the other.
...
The concept of surfeit can be extended to research areas, to quantitatively measure the amount of extra effort we are using to describe the topics of the area.

\begin{definition}
Let $A \subset \mathcal{E}$ be an area with known subset $\hat{A} = \{r_1, r_2, \ldots, r_n\}$, and let $d_{\hat{A}}$ be a description. We define the \emph{surfeit of the description} $d_{\hat{A}}$ as:
\[
\sigma \left( d_{\hat{A}} \right) = 1  - \frac{K( \langle r_1, r_2, \ldots, r_n \rangle )}{l \left( d_{\hat{A}} \right)}
\]
\end{definition}
...
\begin{definition}
Let $A \subset \mathcal{E}$ be an area with known subset $\hat{A} = \{r_1, r_2, \ldots, r_n\}$, and let $d_{\hat{A}}$ be a description. We define the \emph{weak redundancy of the description} $d_{\hat{A}}$ as:
\[
...
The concept of redundancy has also been investigated in the context of information theory, since we are interested on using codes with low redundancy (see for example \cite{abramson1963information}).
