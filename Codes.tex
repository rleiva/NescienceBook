...
\begin{definition}
A code $C$ is \emph{compact}\index{Compact code} if, for a given source alphabet $\mathcal{S}$ and a corresponding probability distribution, its expected length $L_{C}$ is minimized among all possible codes that can be constructed for $\mathcal{S}$.
\end{definition}
