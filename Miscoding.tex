...
\label{def:miscoding}
Let $r \in \mathcal{R}$ be a string. We define the \emph{miscoding} of $r$, denoted by $\mu(r)$, as:
\[
...
\begin{proposition}
For any valid representation $r^\star_e \in \mathcal{R}^\star_\mathcal{E}$, it holds that $\mu\left( r^\star_e \right) \leq \mu\left( r \right)$ for all $r \in \mathcal{R}$.
\end{proposition}
\begin{proof}
This follows from the fact that $\mu\left( r^\star_e \right) = 0$ and $\mu\left( r \right) \geq 0$ for all $r \in \mathcal{R}$.
\end{proof}
...
Since the concatenated string $st \in \mathcal{B}^\star$, the miscoding function can be applied to it without introducing a separate definition. The miscoding of the joint representation is given by:
\[
...
The concept of miscoding can be naturally extended to joint representations formed from any finite collection of representations. Let $r_1, r_2, \ldots, r_n \in \mathcal{R}$ be a finite collection of representations. The miscoding of their concatenation, denoted by $r_1 r_2 \ldots r_n$, is defined as:
\[
...
\]
As in the case of concatenating two representations, the miscoding of a joint representation composed of $n$ elements remains bounded within the unit interval $0 \leq \mu(r_1 r_2 \ldots r_n) \leq 1$. However, several caveats must be noted. There is no guarantee that $\mu(r_1 r_2 \ldots r_n) \leq \mu(r_i) \quad \text{or} \quad \mu(r_1 r_2 \ldots r_n) \geq \mu(r_i)$ for any $i = 1, \ldots, n$. Moreover, the miscoding of the joint representation is not necessarily less than or equal to the sum of the individual miscodings $\mu(r_1 r_2 \ldots r_n) \leq \mu(r_1) + \mu(r_2) + \cdots + \mu(r_n)$. Finally, no general conclusions can be drawn regarding how the miscoding of a joint representation is affected by permutations of its components. That is, reordering the representations may increase, decrease, or leave unchanged the overall miscoding.
...
\[
\mu(r)= \overset{o} { \min_{r_{e}^{\star}\in\mathcal R^{\star}_{\mathcal E}} }
...
\begin{definition}
Let $r \in \mathcal{B}^\ast$ be a string. If $\mu(r) = 1$ we say that $r$ is a \emph{targetless}\index{Targetless representation} representation.
\end{definition} 
...
\begin{proposition}
For every finite or countably infinite set of entities $\mathcal{E}$, there exists an countably infinite set of targetless representations.
\end{proposition}
\begin{proof}
...
For every $n$-fold representation $R = \left( r_1, r_2, \ldots, r_n \right) \in \mathcal{B}^\ast \times \mathcal{B}^\ast \times \cdots \times \mathcal{B}^\ast$, it holds that $0 \leq \mu(R) \leq 1$.
\end{proposition}
\begin{proof}
