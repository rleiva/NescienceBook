...
Let $\mathcal{E}$ be a set of entities. A \emph{representation function}, or \emph{oracle}, is a function $f_\mathcal{E}:\mathcal{B}^\ast \rightarrow \mathcal{E}$ from the set $\mathcal{B}^\ast$ of all possible finite binary strings to the set $\mathcal{E}$ of entities under study.
\end{definition}
...
\begin{example}
Given a set of entities $\mathcal{E}$, if a representation function $f_\mathcal{E}$ exists, it is not unique. For example, a binary not oracle (transform zeros into ones and the other way around) that assigns to each string $x \in \mathcal{B}^\ast$ the entity $f_\mathcal{E} \left( \neg x \right)$ would be also a representation function. Which oracle is the best is something that depends on the area in which the theory of nescience is being applied.
\end{example}

According to the theory of nescience, scientific research would be not only about figuring out how to properly encode entities, but also about discovering the inner workings of the decoding oracle. The capacity of the oracle of reconstruct the original entities is what justifies the fact that we can formulate hypotheses about entities given their representations (surrogative reasoning).
