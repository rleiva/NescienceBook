...
We distinguish between \emph{knowable} and \emph{unknowable} entities. Knowable entities are those that can, in principle, be understood through scientific inquiry (see Section \ref{sec:what-is-an-entity}). Unknowable, relative to the selected oracle, means that no finite representation is mapped to the entity.
...
\begin{definition}
Let $e \in \mathcal{E}$ be a knowable entity. We define the \emph{set of valid representations}\index{Set of valid representations} for $e$, denoted by $\mathcal{R}^\star_e$, as $\{ r \in \mathcal{B}^\ast : \mathcal{O}_\mathcal{E} (r) = e \}$.
\end{definition}
...
A consequence of working with finite strings as representations is that some entities may not be encoded by any representation (see the non-gray areas in Figure \ref{fig:entities_topics_1}; in particular, the entity $e_2$ is not encoded by any string). Intuitively, this reflects the fact that, in certain domains of knowledge, the number of possible problems far exceeds the number of available solutions.
...
Since our knowledge of the entities and the internal workings of the oracle is incomplete, in practice we work with a different set of strings, denoted by $\mathcal{R}_e$, which we believe are close approximations of the true representations $\mathcal{R}^\star_e$ that encode the entity $e$. The elements of this set typically change over time, as our understanding of the entities in $\mathcal{E}$ and of how the oracle encodes them improves. The more abstract the set of entities is, the more difficult it becomes to approximate them as strings (see Chapter \ref{chap:Miscoding}).
...
Once a distance or correction criterion is chosen, one may associate invalid strings with nearby valid representations (see \ref{chap:Miscoding}).

The oracle defines an equivalence relation that partitions the set $\mathcal{B}^\ast$ into equivalence classes. Each class corresponds to an entity in $\mathcal{E}$, although it may happen that some entities in $\mathcal{E}$ do not have any associated class, these are what we call the unknowable unknowns\index{Unknowable unknown}. Not every string is associated with some entity, and some representations are better than others (i.e., they yield lower nescience). No string can represent more than one entity. Moreover, for any given collection $\mathcal{E}$, multiple valid oracles may exist.

\begin{definition}
Let $dom(\mathcal{B}^\ast) / \mathcal{O}$, where $r \sim s$ if and only if $\mathcal{O}_\mathcal{E}(r) = \mathcal{O}_\mathcal{E}(s)$, be the quotient set defined by the oracle's equivalence relation over the set of binary strings. We call each class in this quotient set an \emph{entity class}\index{Entity class}.
\end{definition}
...
The concept of joint representation is defined for any pair of strings $s, t \in \mathcal{R}_\mathcal{E}$, even if they do not belong to the same set of representations $\mathcal{R}_e$, that is, even when $\mathcal{O}_\mathcal{E}(s) \neq \mathcal{O}_\mathcal{E}(t)$. This ability to combine representations of different entities will prove particularly useful for the discovery of new research entities that are currently unknown (see Section \ref{sec:new-topics}).

The operation of string concatenation is associative; that is, for all $r, s, t \in \mathcal{R}_\mathcal{E}$, we have $(rs)t = r(st)$.
...
Descriptions are strings that satisfy the additional requirement of being computable. Descriptions and representations are both finite binary strings, but they play different formal roles. In practice, it might happen that there axists descriptions that describe other descriptions. However, it is not advisable to use descriptions as representations of entities, since what we seek in a good representation is the inclusion of as many details as possible about the original entities, not a concise encoding. Using descriptions in place of representations would make the task of scientific discovery considerably more difficult for humans.
...
There is little value in descriptions that are longer than the representations they describe, that is, descriptions that do not compress the representations.
...
The converse of Proposition \ref{prop:description_implies_conditional} is not true. The fact that $d$ is a conditional description (i.e., $d \in \mathcal{D}_{r \mid s}$) does not imply that $d$ is also a valid description (i.e., $d \in \mathcal{D}_{r})$. Indeed, while we require that $TM \left(\langle a, s \rangle \right) = r$, we do not require that $TM(a) = r$, and in general, this may not hold.
...
\begin{definition}
Let $\mathcal{A} \subset \mathcal{E}$ be a research area. We define the \emph{known subset of the area}\index{Known subset of an area} $\mathcal{A}$, denoted by $\hat{\mathcal{A}}$, as the set of entities $e_1, e_2, \ldots, e_n \in \mathcal{A}$ for which at least one non-pleonastic description is known.
\end{definition}
...
\begin{definition}
Let $R_{\hat{\mathcal{A}}} = \{ r_1, r_2, \ldots, r_n \}$ be the representation of an area $\mathcal{A}$. We call a \emph{description of the area $\mathcal{A}$}\index{Description of area} given the known subset $\hat{\mathcal{A}}$, abbreviated as \emph{description of $\mathcal{A}$}, and denoted by $d_{\hat{\mathcal{A}}}$, to any string in the form $\langle TM, a\rangle$ such that the Turing machine $TM$, when given input $a$, outputs the sequence $\langle r_1, r_2, \ldots, r_n \rangle$.
\end{definition}
