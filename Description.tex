...
The contents of $\mathcal{E}$ depend on the specific domain in which the theory of nescience is applied and are typically aligned with a particular area of knowledge. Examples of collections of entities include research components in mathematics (abstract entities), the biological kingdom \emph{Animalia} (living entities), known and unknown human needs (abstract entities), or the class of all potential computer programs (strings). From a formal perspective, $\mathcal{E}$ should be regarded as a determinate but epistemically inaccessible collection: although its elements are fixed by the underlying domain, there is, in general, no currently available rule or effective procedure for deciding whether a given object belongs to $\mathcal{E}$, nor for enumerating its elements.

The abstract nature of $\mathcal{E}$ offers certain advantages, but also imposes significant restrictions. The main restriction is that our definition of nescience is, in most cases, a non-computable quantity in the classical sense, and therefore cannot be evaluated exactly by effective procedures, requiring the use of approximations in practice. NevertNevertheless, nescience is well defined with respect to an idealized standard that determines whether a representation contains enough information, a standard that will be introduced formally in the next sectionheless, nescience is well defined relative to an idealized reference notion of representation adequacy, which will be introduced formally in the next section. The principal advantage of this abstraction is that the concepts and methods presented in this book can be applied to a wide range of problems, not limited solely to the discovery of new scientific knowledge.

In the theory of nescience, the possibility of using universal sets is excluded; that is, the existence of a set $\xi$ containing everything cannot be assumed. The problem with universal sets is that they contravene Cantor's theorem (see Example \ref{cantor_theorem}). Cantor's theorem proves that the power set $\mathcal{P}(\xi)$, consisting of all possible subsets of $\xi$, has more elements than the original set $\xi$. This contradicts the assumption that $\xi$ includes everything. In the theory of nescience, the set $\mathcal{E}$ must be a specific set.
...
Given a collection of entities \(\mathcal{E}\), a \emph{representation oracle}\index{Representation oracle}
is a total oracle function
\[
\mathcal{O}_{\mathcal{E}} : \mathcal{B}^\ast \longrightarrow \{\mathrm{yes},\mathrm{no}\}
\]
...
\[
\mathcal{O}_{\mathcal{E}}(r)=\mathrm{yes}
...
Rather than deploying individual oracles, we could have employed a universal oracle machine. This is a machine $\mathcal{U}_\mathcal{O}$ that, given the encoding of an oracle $\mathcal{O}_\mathcal{E}$ and a string $r$ as input, computes $\mathcal{U}_\mathcal{O} \left( \langle \mathcal{O}_\mathcal{E}, r \rangle \right) = \mathcal{O}_\mathcal{E} \left( r \right)$. Universal machines are particularly applicable to the universal set $\xi$, encompassing all entities. However, such an approach would complicate the process of scientific discovery in practical terms. Instead, we work with entity sets $\mathcal{E}$ corresponding to different areas of knowledge, each associated with a single oracle $\mathcal{O}_\mathcal{E}$, the one most suitable to our current knowledge and practical needs.
...
\begin{definition}
We say that an entity $e \in \mathcal{E}$ is \emph{knowable}\index{Knowable entity} if there exists at least one $r \in \mathcal{B}^\ast$ such that $\mathcal{O}_\mathcal{E}(r) = true$. An entity $e \in \mathcal{E}$ is \emph{unknowable}\index{Unknowable entity} if it is not knowable.
\end{definition}
...
For scientific inquiry to be practically effective, the oracles should ideally be minimal in size. A \emph{non-minimal oracle}\index{Non-minimal oracle} is one in which part of the information needed to reconstruct the original entities is embedded within the oracle internal representation, rather than being explicitly encoded in the representation string. Although using non-minimal oracles can simplify the research process by offloading complexity to the oracle, it obscures the boundary between what is encoded and what is assumed, making it harder to analyze or generalize the representation mechanism.

Our objective should be to employ representations that minimize the size of the oracle, that is, the amount of prior knowledge presumed to be embedded within it.

\begin{example}
Consider the task of representing chemical compounds. A non-minimal oracle could internally assume the entire body of modern chemical knowledge, including the periodic table, standard valences, and common molecular structures. Under such an oracle, a representation string might consist solely of a compound's common name, such as "water" or "ethanol," and the oracle would reconstruct the full molecular structure using its embedded knowledge. In this case, most of the information required for reconstruction is not contained in the representation itself, but is instead stored implicitly within the oracle.

By contrast, a more minimal oracle would require representations that explicitly encode structural information, for example by specifying the constituent atoms and their bonds. Here, the burden of description lies primarily in the representation string rather than in the oracle. While such representations may be longer or more complex, they make explicit what is being encoded and reduce reliance on unarticulated background assumptions.
\end{example}

Recall that, within the theory of nescience, representations are not required to be minimal. Their primary role is to be correct, in the sense that they contain sufficient information for the oracle to reconstruct the corresponding entities. Representations themselves are not intended to support scientific reasoning directly, and therefore need not be concise. The requirement of minimality becomes relevant only at a later stage, when we introduce descriptions, or models (see Section \ref{sec:descriptions_models}). Descriptions aim to capture essential aspects of representations in a more compact form and are explicitly designed to support reasoning, explanation, and prediction. In that context, assumptions about background knowledge and shared conventions are not only acceptable but often necessary to achieve brevity and explanatory power.
...
%
% Section: Perfect Representations
%
\section{Perfect Representations}
...
\begin{definition}
\label{def:Re}
Given a fixed entity $e \in \mathcal{E}$, we define the set of \emph{perfect representations of $e$} as
\[
\mathcal{R}_e
\;:=\;
\bigl\{\, r \in \mathcal{B}^\ast \;\big|\; \delta_{\mathcal{E}}(r) = e \,\bigr\}.
\]
\end{definition}

The set $\mathcal{R}_e$ is generally unknown and need not be decidable or enumerable; its definition is semantic and oracle-relative, and does not imply that membership in \(\mathcal{R}_e\) can be effectively tested.


We begin by distinguishing between \emph{perfect representations}, strings that contain all the information required by a selected oracle to reconstruct an entity, and \emph{non-perfect representations}. In our framework, the only tool available to make this distinction is an abstract representation oracle $\mathcal{O}_{\mathcal{E}}$, which determines whether a given string constitutes a valid representation of some entity in the set $\mathcal{E}$.

Recall from Section~\ref{sec:representations} that we address the scientific representation problem by introducing an abstract oracle $\mathcal{O}_{\mathcal{E}} : \mathcal{B}^\ast \rightarrow \{\mathrm{yes},\mathrm{no}\}$,
which evaluates finite binary strings and indicates whether each string contains sufficient information, according to an idealized standard, to reconstruct a unique entity in $\mathcal{E}$. The internal mechanism by which the oracle performs this evaluation is not accessible and need not be specified.

\begin{definition}
Let \(\mathcal{E}\) be a collection of entities, and let $\mathcal{O}_{\mathcal{E}} : \mathcal{B}^\ast \to \{\mathrm{true},\mathrm{false}\}$ be a representation oracle. The set of \emph{perfect representations}\index{Perfect representation} for \(\mathcal{E}\) with respect to \(\mathcal{O}_{\mathcal{E}}\) is defined as
\[
\mathcal{R}^\star_{\mathcal{O}_{\mathcal{E}}}
\;=\;
\bigl\{\, r \in \mathcal{B}^\ast \;\big|\; \mathcal{O}_{\mathcal{E}}(r)=\mathrm{true} \,\bigr\}.
\]
\end{definition}

The set $\mathcal{R}^\star_{\mathcal{O}_{\mathcal{E}}}$ is generally unknown, since its definition depends on the behavior of an abstract and typically inaccessible oracle. Intuitively, a representation is considered perfect if it contains exactly the information required for the oracle to reconstruct the corresponding entity without relying on any additional information. A perfect representation must neither include incorrect or spurious information nor omit any information essential for reconstruction; otherwise, the oracle would fail to recognize it as valid.

Within this framework, only finite binary strings are admitted as representations of entities. Drawings, mathematical formulas, experimental datasets, or other artifacts may serve as representations only insofar as they can be encoded as binary strings and are accepted by the oracle as perfect representations. When the oracle returns $\mathrm{true}$ for a string $r$, that string is assumed to determine a unique entity. Different representation oracles may induce different sets of perfect representations, reflecting different idealized standards of reconstruction. Throughout this work, we assume that a specific representation oracle $\mathcal{O}_{\mathcal{E}}$ has been fixed for the collection $\mathcal{E}$ under study.

\begin{notation}
When the choice of oracle is clear from context, we denote the set of perfect representations simply by $\mathcal{R}^\star_{\mathcal{E}}$.
\end{notation}

As illustrated in Example~\ref{ex:description_dna}, an entity may admit more than one perfect representation, even when the oracle is restricted to a particular representational style. It may also occur that an entity admits no perfect representation at all; in such cases, the entity is, within our framework, unknowable.

\begin{notation}
We write $r^\star_e \in \mathcal{R}^\star_{\mathcal{E}}$ to indicate that the string $r$ is a perfect representation of the entity $e$, whenever such a string is known.
\end{notation}

In practice, the representation available to us for an entity $e \in \mathcal{E}$ is often imperfect. That is, instead of working with an unknown perfect representation $r \in \mathcal{R}^\star_{\mathcal{E}}$, we work with a different string $r' \in \mathcal{B}^\ast$ that approximates $r$ but does not encode the entity exactly. A central objective of this book is to quantify the discrepancy between $r'$ and a perfect representation $r$ as a measure of the error introduced by miscoding (see Chapter~\ref{chap:Miscoding}). In general, this discrepancy cannot be computed exactly, since perfect representations are typically unknown and there is no computable function mapping entities to their representations.

\begin{definition}[Kolmogorov-tolerant representation oracle]
\label{def:kolmogorov_tolerant_oracle}
Let $\mathcal{O}_{\mathcal{E}} : \mathcal{B}^\ast \to \{\mathrm{true},\mathrm{false}\}$ be a representation oracle and let
\[
\mathcal{R}^\star_{\mathcal{E}}
=
\{\, r \in \mathcal{B}^\ast \mid \mathcal{O}_{\mathcal{E}}(r)=\mathrm{true} \,\}.
\]
Fix a constant $c \ge 0$. The \emph{Kolmogorov-tolerant representation oracle} is the total function
\[
\mathcal{O}^{(c)}_{\mathcal{E}} : \mathcal{B}^\ast \to \{\mathrm{true},\mathrm{false}\}
\]
defined by
\[
\mathcal{O}^{(c)}_{\mathcal{E}}(r)=\mathrm{true}
\quad\Longleftrightarrow\quad
\Bigl(\mathcal{O}_{\mathcal{E}}(r)=\mathrm{true}\Bigr)
\ \vee\
\Bigl(\exists r^\star \in \mathcal{R}^\star_{\mathcal{E}}:\ K(r^\star \mid r) \le K(r^\star)-c\Bigr).
\]
\end{definition}

\begin{example}
The astronomical records used in the time of Ptolemy to describe the positions of celestial bodies throughout the year constituted an imperfect encoding of the entity "positions of celestial bodies," due to systematic observational inaccuracies. More accurate encodings were later obtained through the improved observations of Tycho Brahe, and modern astronomy provides even higher-precision representations. By contrast, appending irrelevant information, such as the name of the observer, to an otherwise accurate dataset would yield a representation that includes non-essential information.
\end{example}

Because our knowledge of both the entities in $\mathcal{E}$ and the oracle's internal criteria is incomplete, in practice we work with a set of candidate strings, denoted by $\hat{\mathcal{R}}_e$, which we regard as approximations to the (unknown) set of perfect representations associated with the entity $e$. The elements of $\hat{\mathcal{R}}_e$ typically evolve over time as scientific understanding advances and as representational practices improve. In general, the more abstract the entities under consideration, the more difficult it becomes to approximate them accurately by finite strings (see Chapter~\ref{chap:Miscoding}).

A direct consequence of working with approximate representations is that some candidate strings may, unbeknownst to us, encode an entity different from the one we intend to study.
...
It may even occur that the representations we employ are intended to encode an entity that does not exist.
