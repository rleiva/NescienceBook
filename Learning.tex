...
There is a great controversy between statisticians and computer scientists about the difference between statistical inference and machine learning. From our point of view {\color{red} there exists} important differences. In statistical inference, given a target variable $y$ and a training data $X$, the goal is to find a model that select the value of $y$ that maximizes $P(y \mid X)$; meanwhile, in machine learning, finding the value with the highest probability is not necessarily our objective, perhaps we are more interested in minimize the number of false positives, or minimize the number of errors among under represented categories. Another difference is that in statistical learning our models are usually mathematical functions that can be manipulated algebraically; however, in machine learning the models are computer programs that do not admit and easy symbolic manipulation. Finally, in statistics we are also interested in the mathematical properties of the methods in use, that is, if there exists a solution, if we can guarantee that search algorithms converge, and to estimate how far the selected model is from the real one; in machine learning our interest is mostly finding models that work in practice\footnote{Of course, statistical inference methods also work in practice, what we means is that statisticians are interested in other properties of the statistical methods, beyond its practical utility.}. 
