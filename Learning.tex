...
\emph{Statistical inference} is the branch of probability that allows us to make reasoned conclusions about an underlying population by analyzing data. From predicting election outcomes to estimating the effectiveness of new drugs, statistical inference plays a crucial role in decision-making under uncertainty. While descriptive statistics summarize and describe the main features of observed data, statistical inference takes this further by leveraging probability models to draw conclusions about the larger population.

By utilizing data collected from samples, statistical inference helps us refine these probabilistic models and estimate unknown parameters that describe the population. Unlike probability theory, which forms the foundation by providing the rules for handling uncertainty, statistical inference applies these rules to real-world data, turning theory into practice.

In this section, we focus on parameter estimation within probabilistic models. Two primary approaches to estimation will be explored: maximum likelihood estimation and Bayes estimators. While other techniques such as hypothesis testing and experimental design are also part of statistical inference, these are beyond the scope of this chapter and are not required for the purposes of this book.
...
The posterior distribution represents our updated knowledge of the parameter $\theta$ after taking into account the observed data.
...
\begin{proposition}
Let $\theta$ be a real-valued parameter. Suppose that the squared error loss function is used and that the posterior mean of $\theta$, $E\left(\theta\mid\mathbf{X}\right)$, is finite. Then, a Bayes estimator of $\theta$ is $\delta^{\ast}\left(\mathbf{X}\right)=E\left(\theta\mid\mathbf{X}\right)$.
\end{proposition}

\begin{proposition}
When the absolute error loss function is used, a Bayes estimator of a real-valued parameter $\delta^{\ast}\left(\mathbf{X}\right)$ equal to a median of the posterior distributin of $\theta$.
\end{proposition}
