...
As in Chapter \ref{chap:Probability_Theory}, in this section we are only considering parametric discrete random variables defined over discrete probability spaces.
...
When assessing the performance of a machine learning model, we want to quantify how close the model’s predictions $\hat{Y}=\hat{f}(X)$ are to the true responses $Y=f(X)+\epsilon$. The error term $\epsilon$ accounts for factors that have not been included in the model and for inherent randomness in the data. 
...
The \emph{Minimum Description Length} principle builds on the fact that, using an optimal code, the length of encoding $H \wedge E$ as a binary string is given by $- \log_2 P ( H \wedge E )$ (see Theorem \ref{th:optimal_codes}). That is:
\begin{equation*}
\begin{split}
...
Like MDL, MML represents the communication of a dataset $D$ as a two-part message:
\begin{enumerate}
...
\begin{definition}
\label{def:multiobjective_dominance}
...
\begin{definition}
The set of Pareto optimal solutions, denoted by $\mathbf{P}_D$, is called the \emph{Pareto optimal set}. The set of Pareto optimal solutions in the space of objectives, denoted by $\mathbf{P}_O$, is called the \emph{Pareto frontier}.
\end{definition}
