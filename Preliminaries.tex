...
The frequentist interpretation of probability proposes to throw the dice a large number of times and compute the releative frequency of even numbers with respect to the total number of throws. In general, the idea is to repeat an experiment a sufficent large number of times under similar conditions. Of course, two new problems appears. First, it is not defined what it means to repeat an experiment under similar conditions, since if we use exactly the same conditions the results will be the same. The second one is that it is not defined what it means a "large number of times". From a practical point of view we have that some experiments cannot repeate a large number of times, for example, what it is the probability that a patient will die if he gets a new experimental drug? A more subtle limitation of the frequentist interpretation is that technically speaking we are dealing with the probability of a succession of experiments, not about the probability of an individual outcome.
