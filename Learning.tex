...
Pareto optimality can be studied from the point of view of decision variable space or objective space. An objective vector is Pareto optimal if its corresponding decision vector is Pareto optimal.
...
In practice it is convenient to normalize the range of the objective values, so that those points closer tto the ideal vector do not receives more importance. A common normalization term is $\mathbf{z}_i^{nad} - \mathbf{z}_i^\star$.
