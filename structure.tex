...
\usetikzlibrary{calc}
\usetikzlibrary{arrows.meta} % rleiva
\usetikzlibrary{automata, positioning} %  rleiva

% Activate the pgfplots package - rleiva
\usepackage{pgfplots}
\pgfplotsset{compat=1.17}  % Adjust the compatibility mode if necessary
\usepackage{xcolor}  % Required for the HSL color model
...
% rlevia, this does not work with Octavo
...
% % Octavo
% \node[anchor=north west,inner sep=0pt] at (\Gm@lmargin,-\Gm@tmargin) {\includegraphics[width=\textwidth]{\thechapterimage}};
% % A4
...
{%
  \begin{tikzpicture}[remember picture,overlay]
    ...
    % Chapter image fitted to the printable area
    \node[anchor=north west,inner sep=0pt]
      at (\Gm@lmargin,-\Gm@tmargin)
      {\includegraphics[width=\textwidth]{\thechapterimage}};

    % Title box fitted to the printable area
    \draw[anchor=north west]
      % (\Gm@lmargin,-5cm)
      (\dimexpr\Gm@lmargin+0.25cm\relax,-5cm)
      node[
        line width=2pt,
      ...
      {%
        ...
        \parbox{\dimexpr\textwidth-0.5cm-30pt\relax}{%
...
{%
  \begin{tikzpicture}[remember picture,overlay]
    ...
    \draw[anchor=north west]
      % (\Gm@lmargin,-6cm)
      (\dimexpr\Gm@lmargin+0.25cm\relax,-6cm)
...
{%
  \begin{tikzpicture}[remember picture,overlay]
    ...
    % Unnumbered chapter title box fitted to the printable area
    \draw[anchor=north west]
      % (\Gm@lmargin,-5.2cm)
      (\dimexpr\Gm@lmargin+0.25cm\relax,-5.2cm)
