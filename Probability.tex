...
When working with random variables it is common to use the name expeced value instead of mean. However, this name is a little bit misleading, since for the majority of the discrete distributions, the expected value is not one of the possible values of the distribution, i.e., the expected value is not expected at all. For example, if we throw a dice, the expected value would be 3.5. This undesired property of the expectation creates a lot of confusion in scientific research.

The expectation of a random variable has a physhical interpretation as the center of gravity of the distribution. A problem with the expectation is that it can be affected greatly by a small change in the probability assigned to a large value of $x$.

The expectatin of the linear combination of $n$ random variables is the linear combination of their expectations.

\begin{proposition}
Let $X_{1}, \ldots, X_{n}$ be $n$ independent random variables such that the expectations $E\left(X_{i}\right)$ are fintite, and let $a_1, \ldots, a_n$ and $b$ constants, then we have that
\[
...
\begin{proposition}
Let $X_{1}, \ldots, X_{n}$ be $n$ independent random variables such that the expectations $E\left(X_{i}\right)$ are finite, then:
\[
