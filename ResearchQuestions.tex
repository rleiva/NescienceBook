%
% CHAPTER: Discovering the Unknown
%
...
In this chapter, we are going to see how to apply our methodology in practice for the assisted discovery of interesting research questions. As was the case in the previous chapter, in which we studied the concept of nescience from a practical point of view, here we will focus on the empirical side of our framework, showing how the theoretical concepts introduced earlier can be operationalized and used to guide actual research activities.

In the first part of this chapter, we will examine how to approximate in practice the two new metrics introduced Chapter \label{chap:Interesting-Research-Questions}: relevance and applicability. The relevance of a topic will be estimated from the number of external backlinks to the topic's Wikipedia page (external links). Applicability will be estimated from the number of incoming internal links from other Wikipedia pages (internal links). We will illustrate both metrics with examples drawn from the set of topics that make up the research area of theoretical computer science.
...
As an approximation of the relevance of a topic, we have used the number of links (URLs) from external web pages on the Internet that point to the topic's page on Wikipedia. The rationale is that the more relevant a topic is, the more people will discuss it on the Internet, and the more URLs will link to its Wikipedia page, since Wikipedia is a well-known source of information that many people cite. In fact, we are not interested in knowing the absolute relevance of research topics; what we need is a measure of \emph{relative} relevance between different topics. An underestimate of the relevance is not harmful as long as this underestimation is distributed uniformly across all topics. Further research is needed to understand how accurately this URL-counting procedure approximates the theoretical concept of relevance. 

Another difficulty is that we do not know the exact number of links to a web page on the Internet. Therefore, we apply a second approximation. In this case, the number of links is estimated using Google's \texttt{link:} search operator. This operator lists the links that Googlebot has discovered during its crawling and indexing process. For example, the number of external pages that link to the \emph{Computer Science} article on Wikipedia can be queried as follows:
...
As Google acknowledges on its website, not all links to a page may be listed. The number of reported links may vary due to redirections, crawling errors, and other factors. How these errors affect the accuracy of link counting is not clear, since the details of Google's crawling algorithm are not public.

Table \ref{tab:Relevance-of-Topics} lists the ten most relevant topics according to this metric. For each topic, it shows its raw relevance (number of external links) and its normalized value. The list includes basic concepts (\emph{binary number}, \emph{floating point}), advanced concepts (\emph{Turing machine}, \emph{finite-state machine}, \emph{cellular automaton}, \emph{lambda calculus}, \emph{Turing completeness}), widely used tools (\emph{recursion}, \emph{regular expression}), and classical problems (\emph{halting problem}). All these topics align well with our intuitive idea of highly relevant concepts, although some readers may question whether topics such as \emph{floating point} belong among the most relevant topics in the theory of computation.
...
Roughly three-quarters of the theoretical computer science topics attract fewer than about 50 incoming Wikipedia links, while only a few reach the triple-digit range and just one or two exceed 300. This steep drop-off is is consistent with a heavy-tailed distribution: applicability, as measured here, is concentrated in a tiny set of "super-connectors," while the median topic sees only modest reuse. In practical terms, the average link count is a misleadingly optimistic figure—pulled upward by a few giants—whereas the typical topic remains relatively niche.

Such inequality has several implications. First, it reinforces the idea that a small core of foundational concepts underpins a large share of problem-solving across the field; investing effort in those hubs yields the greatest leverage. Second, the near-emptiness of the mid-range suggests that rising topics face a kind of applicability transition gap: they must bridge a substantial gap before joining the elite group of widely referenced concepts. Finally, the long tail highlights opportunity—numerous specialized notions lie at low link counts, potentially poised for breakout if new cross-disciplinary applications emerge.

Table \ref{tab:Applicability-of-Topics} lists the ten most applicable topics according to this metric. The backlink data confirm the long-tailed distribution: applicability, at least as proxied by Wikipedia's “What links here” counts, is highly unequal. A single super-hub (the \emph{Dirac delta function}, 625 links) dwarfs the rest, while only a handful of topics even cross the 200-link mark. This means that when researchers look for broadly reusable tools, the "return on attention" is greatest in a very small set of concepts that already function as connective tissue across many areas.
...
As introduced in Definition \ref{sec:intro_maturity}, \emph{maturity} is the inverse of \emph{surfeit}. While surfeit measures the amount of redundant or superfluous information present in our current description of a topic, maturity quantifies how concise and efficient that description has become. In other words, a topic is considered mature when our understanding of it is sufficiently advanced that its essential content can be stated without unnecessary verbosity. High maturity thus indicates that knowledge about the topic has been largely consolidated, leaving little room for removing redundant parts from its description.

The maturity of a topic is estimated based on the length of its Wikipedia article (only the textual content) and the length of a compressed version of that same text. The intuition behind this approach is that the more mature a topic is, the less redundant information its article will contain, and therefore the less compressible it should be. Topics that are still immature often accumulate repetitive content, which facilitates compression. By comparing the original and compressed lengths, we obtain a practical proxy for surfeit (as the ratio between the two lengths), and therefore for maturity as its inverse.
...
If we combine highly relevant, poorly understood problem topics pairwise, we might obtain new topics that lie in the \emph{unknown unknown} area.
