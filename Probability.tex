...
Let $\left( \Omega, \mathcal{A} , P \right)$ be a probability space. A \emph{probability function}\index{Probability function} is a real value funcion $f : \mathcal{A} \rightarrow [0, 1]$ such that for each $A \in \mathcal{A}$ we have that $f \left( A \right) = P \left( A \right)$.
\end{definition}
