...
\end{proposition}
\begin{proof}
Non-negativity follows because complexities are non-negative, and so $NID(s,t)\geq 0$. 
...
Normalizing by $\max\{K(x),K(z)\}$ introduces at most a vanishing additive error. Thus $NID$ satisfies the metric axioms up to negligible terms.
\end{proof}
...
\]
Dividing both sides by $\max\{K(x),K(y)\}$ yields the expression for $NID(x,y)$.
\end{proof}
