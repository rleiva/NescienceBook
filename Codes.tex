...
Fortunately, prefix-free codes are also uniquely decodable, as the next proposition proves.

\begin{proposition}
Let $C$ be a prefix-free code, then $C$ is uniquely decodable.
\end{proposition}
\begin{proof}
Assume that there are two different source words \( r \) and \( s \) such that \( C^n(r) = C^n(s) \) for some extension \( C^n \). Since \( C \) is prefix-free, no complete code word in the sequence \( C^n(r) \) can be a prefix of another, preventing \( C^n(r) \) from being equal to \( C^n(s) \) unless \( r = s \), which is a contradiction. Therefore, \( C \) must be uniquely decodable.
\end{proof}

From an engineering point of view it is highly convenient to have codes whose source symbols can be decoded as soon as the corresponding code words are received, that is, it is not necessary to wait for the next code word in order to decode the current symbol. Prefix-free codes present this property, this is why some authors prefer to call them \emph{instantaneous codes}.
...
Another interesting type of codes are fixed length codes. We will use fixed length codes to compute the length of a text when there are no regularities we can exploit to compress the text.
...
\begin{figure}[h]
\centering\includegraphics[scale=0.4]{PrefixFreeTree2}
