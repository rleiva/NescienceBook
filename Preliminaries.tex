...
{\color{red} Definition of combinatorics as a collection of counting methods}

Multiplication Rule: Suppose than an experiment has $k$ parts $\left( k \geq 2 \right)$, that the $i$th part of the experiment can have $n_{i}$ possible outcomes $\left( i=1, \ldots, k \right)$, and that all of the outcomes in each part can occur regardless of which specific outcomes have occurred in the other parts. Then the sample space $S$ of the experiment will contain all vectors of the form $\left( u_{1}, \ldots, u_{k} \right)$ where $u_{i}$ is one of the $n_{i}$ possible outcomes of part $i \, \left( i=1, \ldots, k \right)$. The total number of these vectors in $S$ will be equal to the product $n_{1} n_{2} \cdots n_{k}$.

Permutations: Suppose that a set has $n$ elements. Suppose that an experiment consists of selecting $k$ of the elements one at a time without replacement. Let each outcome consist of the $k$ elements in the order selected. Each such outcome is called a permutation of $n$ elements taken $k$ at a time. We denote the number of distinct such permutations by the symbol $P_{n.k}$. The number of permutations of $n$ elements taken $k$ at a time is $P_{n,k} = n \left( n-1 \right) \cdots \left( n-k+1 \right)$. When $k=n$, the number of permutations is given by $P_{n,n} = n \left( n-1 \right) \cdots 1=n!$, where the symbol $n!$ is read $n$ factorial.

Stirling's Formula

Theorem 36. (Stirling's Formula) Lets_{n}=\frac{1}{2}\log\left(2\pi\right)+\left(n+\frac{1}{2}\right)\log\left(n\right)-n

Then \lim_{n\rightarrow\infty}\left|s_{n}-\log\left(n!\right)\right|=0. Put another way\lim_{n\rightarrow\infty}\frac{\left(2\pi\right)^{\frac{1}{2}}n^{n+\frac{1}{2}}e^{-n}}{n!}=1

1.8 Combinatorial Methods

Many problems of counting the number of outcomes in an event amount to counting how many subsets of a certain size are contained in a fixed set.

Combinations

Definition 37. Consider a set with n elements. Each subset of size k chosen from this set is called a combination of n elements taken k at a time, denoted by C_{n,k}.

Theorem 38. The number of distinct subsets of size k that can be chosen from a set of size n is:C_{n,k}=\frac{P_{n,k}}{k!}=\frac{n!}{k!\left(n-k\right)!}

Binomial Cefficients

Definition 39. The number C_{n,k} is also denoted by the symbol {n \choose k}. When this notation is used, this number is called binomial coefficient.

Theorem 40. (Binomial Theorem) For all numbers x and y and each positive integer n,\left(x+y\right)^{n}=\sum_{k=0}^{n}{n \choose k}x^{k}y^{n-k}



Theorem 41. For all n,{n \choose 0}={n \choose n}=1

For all n and all k=0,1,\ldots,n,{n \choose k}={n \choose n-k}

It is sometimes convenient to use the expression “n choose k” for the value of C_{n,k}. The general formula for the number of unordered samples of size k with replacement from n elements is {n+k-1 \choose k}. The number of different arrangements of n objects consisting of k similar objects of one type and n-k similar objects of a second type is {n \choose k}.

1.9 Multinomial Coefficients

A multinomial coefficient is a generalization of the binomial coefficient.

Definition 42. The number \frac{n!}{n_{1}!n_{2}!\cdots n_{k}!}, that we shall denote {n \choose n_{1},n_{2},\ldots,n_{k}}, is called multinomial coefficient.

Multinomial coefficients can be used to represent the number of different arrangements of the elements of a set containing elements of k different types (k>2).

Theorem 43. (Multinomial Theorem) For all numbers x_{1},\ldots,x_{k} and each positive integer n\left(x_{1}+\ldots+x_{k}\right)^{n}=\sum{n \choose n_{1},n_{2},\ldots,n_{k}}x_{1}^{n_{1}}x_{2}^{n_{2}}\cdots x_{k}^{n_{k}}

where the summation extends over all possible combinations of nonnegative integers n_{1},\ldots,n_{k} such that n_{1}+n_{2}+\cdots+n_{k}=n.
...
{\color{red} Sipser book for the langugaes part}

{\color{red} Perhaps a book on combinatorics}